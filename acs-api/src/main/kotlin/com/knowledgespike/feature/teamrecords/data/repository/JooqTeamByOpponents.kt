...
object JooqTeamByOpponents {
    ...
    fun createTotalsTable(
        ...
        context.createTemporaryTableIfNotExists(tempTableName).`as`(
            select(
                ...
                .groupBy(field("${scoresTempTableName}.teamid"),field("${scoresTempTableName}.opponentsid"), )
        ).execute()
    }
    ...
    fun createResultsCte(
        ...
        minimumValue: Int,
        isForBattingTeam: Boolean,
    ): SelectConditionStep<Record16<Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, BigDecimal>> {
        ...
        val cte =
            with("cteResults").`as`(
                select(
                    ...
                    rowNumber().over().partitionBy(field("teamid"), field("opponentsId")).`as`("rn"),
                    sum(
                        `when`(
                            field("result").eq(1),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("opponentsId")).`as`("won"),
                    sum(
                        `when`(
                            field("result").eq(2),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("opponentsId")).`as`("lost"),
                    sum(
                        `when`(
                            field("result").eq(4),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("opponentsId")).`as`("drawn"),
                    sum(
                        `when`(
                            field("result").eq(8),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("opponentsId")).`as`("tied"),
                )
                    .from(scoresTempTableName)
            )
                .with("cteLowestScore").`as`(
                    select(
                        ...
                        field("${scoresTempTableName}.opponentsId", Int::class.java),
                        min(field("total", Int::class.java)).`as`("ls")
                    ).from(scoresTempTableName)
                        .join(INNINGS).on(field("${scoresTempTableName}.id").eq(INNINGS.MATCHID))
                        .where(field("${scoresTempTableName}.teamid").eq(compareTeamId))
                        .and(INNINGS.COMPLETE.eq(1))
                        .groupBy(field("${scoresTempTableName}.opponentsId"), field("${scoresTempTableName}.teamid"))
                )
                .select(
                    ...
                    select(
                        ...
                        .and(field("games.opponentsId").eq(field("${totalTableName}.opponentsId")))

                        .leftJoin("cteResults")
                        .on(field("games.teamid").eq(field("cteResults.teamid")))
                        .and(field("games.opponentsId").eq(field("cteResults.opponentsId")))
                        .and(field("rn").eq(1))

                        .leftJoin("cteLowestScore")
                        .on(field("cteLowestScore.teamid").eq(field("${totalTableName}.teamid")))
                        .and(field("cteLowestScore.opponentsId").eq(field("${totalTableName}.opponentsId")))
