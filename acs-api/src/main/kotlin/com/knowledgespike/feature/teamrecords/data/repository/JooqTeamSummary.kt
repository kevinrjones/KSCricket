...
object JooqTeamSummary {
    ...
    fun createResultsCte(
        ...
        minimumValue: Int,
        isForBattingTeam: Boolean,
    ): SelectConditionStep<Record15<Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, BigDecimal>> {
        ...
        val cte =
            with("cteResults").`as`(
                select(
                    ...
                    rowNumber().over().partitionBy(field("teamid")).`as`("rn"),
                    sum(
                        `when`(
                            field("result").eq(1),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid")).`as`("won"),
                    sum(
                        `when`(
                            field("result").eq(2),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid")).`as`("lost"),
                    sum(
                        `when`(
                            field("result").eq(4),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid")).`as`("drawn"),
                    sum(
                        `when`(
                            field("result").eq(8),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid")).`as`("tied"),
                )
                    .from(scoresTempTableName)
            )
                ...
                .select(
                    ...
                    select(
                        ...
                        .leftJoin("cteResults").on(field("games.teamid").eq(field("cteResults.teamid")))
                        .and(field("rn").eq(1))
