...
object JooqTeamByGround {
    ...
    fun createTotalsTable(
        ...
        val groupByFields = JooqTeamHelpers.getGroupingFieldsForSearchParameters(scoresTempTableName, searchParameters, "locationId")
        ...
        context.createTemporaryTableIfNotExists(tempTableName).`as`(
            select(
                ...
                field("${scoresTempTableName}.locationid"),
    ...
    ): SelectConditionStep<Record16<Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, BigDecimal, String>> {
        ...
        val cte =
            with("cteResults").`as`(
                select(
                    ...
                    rowNumber().over().partitionBy(field("teamid"), field("locationid")).`as`("rn"),
                    sum(
                        `when`(
                            field("result").eq(1),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("locationid")).`as`("won"),
                    sum(
                        `when`(
                            field("result").eq(2),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("locationid")).`as`("lost"),
                    sum(
                        `when`(
                            field("result").eq(4),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("locationid")).`as`("drawn"),
                    sum(
                        `when`(
                            field("result").eq(8),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("locationid")).`as`("tied"),
                )
                    .from(scoresTempTableName)
            )
                .with("cteLowestScore").`as`(
                    select(
                        ...
                        field("${scoresTempTableName}.locationid", Int::class.java),
                        min(field("total", Int::class.java)).`as`("ls")
                    ).from(scoresTempTableName)
                        .join(INNINGS).on(field("${scoresTempTableName}.id").eq(INNINGS.MATCHID))
                        .where(field("${scoresTempTableName}.teamid").eq(compareTeamId))
                        .and(INNINGS.COMPLETE.eq(1))
                        .groupBy(field("${scoresTempTableName}.locationid"), field("${scoresTempTableName}.teamid"))
                )
                .select(
                    ...
                    field("ls"),
                    strikeRate.`as`("sr"),
                    concat(GROUNDS.KNOWNAS, `val`(" ("), GROUNDS.COUNTRYNAME, `val`(")")).`as`("ground"),
                    ).from(
                    select(
                        ...
                        .and(field("games.locationid").eq(field("${totalTableName}.locationid")))

                        .leftJoin("cteResults")
                        .on(field("games.teamid").eq(field("cteResults.teamid")))
                        .and(field("games.locationid").eq(field("cteResults.locationid")))
                        .and(field("rn").eq(1))

                        .leftJoin("cteLowestScore")
                        .on(field("cteLowestScore.teamid").eq(field("${totalTableName}.teamid")))
                        .and(field("cteLowestScore.locationid").eq(field("${totalTableName}.locationid")))
                        .join(GROUNDS).on(field("games.locationid").eq(GROUNDS.ID))
