 ...
 */
object JooqBowlingIndividualSeries {

    /**
     * Creates a common table expression (CTE) query used to calculate cricket bowling statistics
     * for players based on the given search parameters and temporary table names.
     *
     * @param searchParameters provide the validated search constraints required to filter the resulting statistics.
     * @param bowlTempName the temporary table name containing bowling information.
     * @param teamsTempName the temporary table name containing the teams data.
     * @param tempMatchCountsName the temporary table name containing match counts data.
     * @return a jOOQ `SelectJoinStep<Record>` object representing the generated CTE query.
     */
    fun createResultsCte(
        ...
        val cte =
            select(
                ...
                field("innings.year", Double::class.java).`as`("year"),
                ...
                select(
                    ...
                    field("${bowlTempName}.seriesNumber"),
                ).from(bowlTempName)
                    ...
                    .and(field("${tempMatchCountsName}.seriesnumber", Int::class.java).eq(field("${bowlTempName}.Seriesnumber", Int::class.java)))
                    .and(field("${tempMatchCountsName}.seriesdate", String::class.java).eq(field("${bowlTempName}.Seriesdate", String::class.java)))
                    .groupBy(field("matchtype"), field("playerid"), field("${bowlTempName}.seriesnumber"), field("${bowlTempName}.seriesdate"))
                    ...
                    .and(
                        field("${teamsTempName}.seriesDate", Int::class.java).eq(
                            field(
                                "innings.year",
                                Int::class.java
                            )
                    ...
                    .and(
                        field("${teamsTempName}.seriesNumber", Int::class.java).eq(
                            field(
