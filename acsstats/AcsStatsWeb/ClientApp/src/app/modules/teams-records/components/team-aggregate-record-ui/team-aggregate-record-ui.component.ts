...
@Component({
    selector: 'app-team-aggregate-record-ui',
    templateUrl: './team-aggregate-record-ui.component.html',
    styleUrls: ['./team-aggregate-record-ui.component.css'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    standalone: false
})
export class TeamAggregateRecordUiComponent implements OnInit {

  @Input() teamUIModel$!: Observable<TeamOverallUiModel>;
  @Input() title!: string;
  ...
  teamSummaryModel$!: Observable<RecordsSummaryModel>;
  sortOrder!: number;
  ...
  private teamUIModelSub$!: Subscription;
  ...
  ngOnDestroy() {
    this.teamUIModelSub$.unsubscribe();
  }

  ngOnInit(): void {
    this.showTeam = true

    this.teamSummaryModel$ = this.teamStore.select(s => {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.teamUIModelSub$ = this.teamUIModel$.subscribe(payload => {
  ...
  getLabelForDraws() {
    return this.recordHelperService.isFirstClass(this.teamSummaryModel$) ? 'Drawn' : 'NR'
  }
}
