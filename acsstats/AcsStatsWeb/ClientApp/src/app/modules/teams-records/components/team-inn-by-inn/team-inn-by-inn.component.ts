...
import {LoadInnByInnTeamRecordsAction} from '../../actions/records.actions';
import {FindRecords} from '../../../../models/find-records.model';

@Component({
    selector: 'app-team-inn-by-inn',
    templateUrl: './team-inn-by-inn.component.html',
    styleUrls: ['./team-inn-by-inn.component.css'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    standalone: false
})
export class TeamInnByInnComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.teamInnByInn$ = this.teamStore.select(s => {
        return s.teamrecords.inningsByInnings
      }
    )
  }

  dispatch(findRecordsPayload: { payload: FindRecords }) {
    this.teamStore.dispatch(LoadInnByInnTeamRecordsAction(findRecordsPayload))
