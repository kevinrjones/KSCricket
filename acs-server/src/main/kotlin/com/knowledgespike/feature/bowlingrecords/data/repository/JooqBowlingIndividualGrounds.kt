 ...
 */
object JooqBowlingIndividualGrounds {

    /**
     * Creates a Common Table Expression (CTE) for retrieving detailed bowling statistics,
     * based on the provided search parameters, using intermediate temporary tables.
     *
     * @param searchParameters Specifies the validated search parameters utilized to filter
     *        and query the bowling statistics, such as match type, teams, and paging configurations.
     * @param bowlTempName The name of the temporary table containing bowling data.
     * @param teamsTempName The name of the temporary table containing team-related data.
     * @param tempMatchCountsName The name of the temporary table containing match count data.
     * @return A `SelectJoinStep<Record>` representing the constructed CTE for compiling
     *         filtered and calculated bowling statistics for further data retrieval.
     */
        ...
        val cte =
            select(
                ...
                select(
                    ...
                    .and(field("${tempMatchCountsName}.locationId", Int::class.java).eq(field("${bowlTempName}.LocationId", Int::class.java)))
                    .groupBy(field("matchtype"), field("playerid"), field("${bowlTempName}.LocationId"))
                    ...
                    .and(field("${teamsTempName}.locationId", Int::class.java).eq(field("innings.locationid", Int::class.java)))
