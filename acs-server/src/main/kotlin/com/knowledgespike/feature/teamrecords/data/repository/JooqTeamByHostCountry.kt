...
object JooqTeamByHostCountry {
    ...
    fun createTotalsTable(
        ...
        val groupByFields = JooqTeamHelpers.getGroupingFieldsForSearchParameters(scoresTempTableName, searchParameters, "homeCountryId")
        ...
        context.createTemporaryTableIfNotExists(tempTableName).`as`(
            select(
                ...
                field("${scoresTempTableName}.homecountryid"),
        ...
        val cte =
            with("cteResults").`as`(
                select(
                    ...
                    rowNumber().over().partitionBy(field("teamid"), field("homecountryid")).`as`("rn"),
                    sum(
                        `when`(
                            field("result").eq(1),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("homecountryid")).`as`("won"),
                    sum(
                        `when`(
                            field("result").eq(2),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("homecountryid")).`as`("lost"),
                    sum(
                        `when`(
                            field("result").eq(4),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("homecountryid")).`as`("drawn"),
                    sum(
                        `when`(
                            field("result").eq(8),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("homecountryid")).`as`("tied"),
                )
                    .from(scoresTempTableName)
            )
                .with("cteLowestScore").`as`(
                    select(
                        ...
                        field("${scoresTempTableName}.homecountryid", Int::class.java),
                        min(field("total", Int::class.java)).`as`("ls")
                    ).from(scoresTempTableName)
                        .join(INNINGS).on(field("${scoresTempTableName}.id").eq(INNINGS.MATCHID))
                        .where(field("${scoresTempTableName}.teamid").eq(compareTeamId))
                        .and(INNINGS.COMPLETE.eq(1))
                        .groupBy(field("${scoresTempTableName}.homecountryid"), field("${scoresTempTableName}.teamid"))
                )
                .select(
                    ...
                    field("ls"),
                    strikeRate.`as`("sr"),
                    field("${totalTableName}.homecountryid").`as`("homecountryid"),
                ).from(
                    select(
                        ...
                        .and(field("games.homecountryid").eq(field("${totalTableName}.homecountryid")))

                        .leftJoin("cteResults")
                        .on(field("games.teamid").eq(field("cteResults.teamid")))
                        .and(field("games.homecountryid").eq(field("cteResults.homecountryid")))
                        .and(field("rn").eq(1))

                        .leftJoin("cteLowestScore")
                        .on(field("cteLowestScore.teamid").eq(field("${totalTableName}.teamid")))
                        .and(field("cteLowestScore.homecountryid").eq(field("${totalTableName}.homecountryid")))
