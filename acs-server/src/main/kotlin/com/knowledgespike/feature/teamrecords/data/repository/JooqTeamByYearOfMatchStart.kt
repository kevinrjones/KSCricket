 ...
 */
object JooqTeamByYearOfMatchStart {
    ...
    fun createTotalsTable(
        ...
        val groupByFields = JooqTeamHelpers.getGroupingFieldsForSearchParameters(scoresTempTableName, searchParameters, "matchstartyear")
        ...
        context.createTemporaryTableIfNotExists(tempTableName).`as`(
            select(
                ...
                field("${scoresTempTableName}.matchstartyear"),
        ...
        val cte =
            with("cteResults").`as`(
                select(
                    ...
                    rowNumber().over().partitionBy(field("teamid"), field("matchstartyear")).`as`("rn"),
                    sum(
                        `when`(
                            field("result").eq(1),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("matchstartyear")).`as`("won"),
                    sum(
                        `when`(
                            field("result").eq(2),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("matchstartyear")).`as`("lost"),
                    sum(
                        `when`(
                            field("result").eq(4),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("matchstartyear")).`as`("drawn"),
                    sum(
                        `when`(
                            field("result").eq(8),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("matchstartyear")).`as`("tied"),
                )
                    .from(scoresTempTableName)
            )
                .with("cteLowestScore").`as`(
                    select(
                        ...
                        field("${scoresTempTableName}.matchstartyear", Int::class.java),
                        min(field("total", Int::class.java)).`as`("ls")
                    ).from(scoresTempTableName)
                        .join(INNINGS).on(field("${scoresTempTableName}.id").eq(INNINGS.MATCHID))
                        .where(field("${scoresTempTableName}.teamid").eq(compareTeamId))
                        .and(INNINGS.COMPLETE.eq(1))
                        .groupBy(field("${scoresTempTableName}.matchstartyear"), field("${scoresTempTableName}.teamid"))
                )
                .select(
                    ...
                    field("${totalTableName}.matchstartyear").`as`("matchStartYear"),
                ).from(
                    select(
                        ...
                        .and(field("games.matchstartyear").eq(field("${totalTableName}.matchstartyear")))

                        .leftJoin("cteResults")
                        .on(field("games.teamid").eq(field("cteResults.teamid")))
                        .and(field("games.matchstartyear").eq(field("cteResults.matchstartyear")))
                        .and(field("rn").eq(1))

                        .leftJoin("cteLowestScore")
                        .on(field("cteLowestScore.teamid").eq(field("${totalTableName}.teamid")))
                        .and(field("cteLowestScore.matchstartyear").eq(field("${totalTableName}.matchstartyear")))
