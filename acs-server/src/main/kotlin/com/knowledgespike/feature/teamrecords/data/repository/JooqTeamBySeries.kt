...
object JooqTeamBySeries {
    ...
    fun createTotalsTable(
        ...
        val compareTeamId = if (isForBattingTeam)
            ...
            INNINGS.OPPONENTSID
        val compareOpponentsId = if (isForBattingTeam)
            INNINGS.OPPONENTSID
        else
            INNINGS.TEAMID
        ...
        context.createTemporaryTableIfNotExists(tempTableName).`as`(
            select(
                ...
                field("${scoresTempTableName}.seriesNumber"),
                ...
                .and(field("${scoresTempTableName}.opponentsId").eq(compareOpponentsId))
                .groupBy(field("${scoresTempTableName}.teamid"),field("${scoresTempTableName}.opponentsid"),field("${scoresTempTableName}.seriesnumber"), )
        ).execute()
    }
    ...
    fun createResultsCte(
        ...
        minimumValue: Int,
        isForBattingTeam: Boolean,
    ): SelectConditionStep<Record17<Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, Any, BigDecimal, Any>> {
        ...
        val cte =
            with("cteResults").`as`(
                select(
                    ...
                    rowNumber().over().partitionBy(field("teamid"), field("seriesNumber")).`as`("rn"),
                    sum(
                        `when`(
                            field("result").eq(1),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("seriesNumber")).`as`("won"),
                    sum(
                        `when`(
                            field("result").eq(2),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("seriesNumber")).`as`("lost"),
                    sum(
                        `when`(
                            field("result").eq(4),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("seriesNumber")).`as`("drawn"),
                    sum(
                        `when`(
                            field("result").eq(8),
                            1
                        ).else_(0)
                    ).over().partitionBy(field("teamid"), field("seriesNumber")).`as`("tied"),
                )
                    .from(scoresTempTableName)
            )
                .with("cteLowestScore").`as`(
                    select(
                        ...
                        field("${scoresTempTableName}.seriesNumber", Int::class.java),
                        min(field("total", Int::class.java)).`as`("ls")
                    ).from(scoresTempTableName)
                        .join(INNINGS).on(field("${scoresTempTableName}.id").eq(INNINGS.MATCHID))
                        .where(field("${scoresTempTableName}.teamid").eq(compareTeamId))
                        .and(INNINGS.COMPLETE.eq(1))
                        .groupBy(field("${scoresTempTableName}.seriesnumber"), field("${scoresTempTableName}.teamid"))
                )
                .select(
                    ...
                    select(
                        ...
                        .and(field("games.seriesNumber").eq(field("${totalTableName}.seriesNumber")))

                        .leftJoin("cteResults")
                        .on(field("games.teamid").eq(field("cteResults.teamid")))
                        .and(field("games.seriesNumber").eq(field("cteResults.seriesNumber")))
                        .and(field("rn").eq(1))

                        .leftJoin("cteLowestScore")
                        .on(field("cteLowestScore.teamid").eq(field("${totalTableName}.teamid")))
                        .and(field("cteLowestScore.seriesNumber").eq(field("${totalTableName}.seriesNumber")))
